# Hi, I'm Sean White 

**Cybersecurity professional transitioning into SOC analysis** — SIEM detection, packet analysis, and identity attack detection. CompTIA Security+ certified, pursuing Microsoft SC-500.

##  Detection Lab Series
- **[SSH Brute Force Detection & Packet Analysis](https://github.com/UscTrojansDodgers56/wireshark-ssh-bruteforce-analysis)** — Sentinel KQL + Wireshark, live attack traffic, MITRE T1110.001
- **Active Directory Attack Detection** — Kerberoasting, AS-REP, DCSync, Pass-the-Hash *(coming soon)*

##  Skills
Microsoft Sentinel · KQL · Splunk · Wireshark · Microsoft Entra ID · Azure · MITRE ATT&CK · Incident Triage

##  Resume
[View my resume](https://github.com/UscTrojansDodgers56/UscTrojansDodgers56/blob/main/Sean_White_Resume.pdf)

##  Connect
[LinkedIn](https://linkedin.com/in/seanwhite56) · [YouTube](https://youtu.be/0Xd_pT5AuJQ)
